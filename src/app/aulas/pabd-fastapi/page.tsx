...
export default function PABDPage() {
  ...
  function goTo(n: number) {
    setCurrent(n)
    if (n >= 2 && !visited.includes(n)) setVisited(prev => [...prev, n])
  ...
  const progressPct = (current / (SECTION_LABELS.length - 1)) * 100
                  ...
                  <div className="quiz-options">
                    {QUIZ[currentQ].opts.map((opt, i) => {
                      ...
                      if (answered) {
                        ...
                        else if (!opt.c && answers[currentQ] === false && i === QUIZ[currentQ].opts.findIndex(o => !o.c && answers[currentQ] !== null)) cls += ' wrong'
                      }
                      // simpler: only mark the clicked option
                      return (
                        <button
                          key={i}
